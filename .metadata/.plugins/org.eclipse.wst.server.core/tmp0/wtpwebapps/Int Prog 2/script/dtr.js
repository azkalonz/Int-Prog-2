(()=>{
	...
	const setLatestRecord = ()=>{
		...
		const lastRecordDate = records[0] && moment(records[0].date_in).format("LL");
		...
		} else if(records[0] && records[0].date_in === ""){
			timeInBtn.addClass("current");
			timeOutBtn.prop("disabled",true);
		} else if(records[0] && isCurrentDate && records[0].date_in !== "" && records[0].date_out !== ""){
			dateInInput.val(moment(records[0].date_in).format("hh:mm A"));
	...
	const init = ()=>{
		...
		$("[name=username]").val(getUser().username);
		$("[name=password]").val(getUser().password);
	}
